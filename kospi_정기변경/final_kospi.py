...
from flow__kospi_calculate import (
    load_data, run_regression,
    REBAL_DATES, SECTORS, REMOVE_OUTLIERS,
)

DATA_PATH      = r'C:\Users\intern9\truston_quant_dev\util\코스피_최종.xlsx'
...
def load_benchmark(filepath: str) -> pd.Series:
    """
    코스피_코스닥_종가.xlsx 에서 코스피 지수 종가를 로딩.
        col 2 (IKS900) = 코스피 종가지수, 데이터 시작 14행

    Returns
    -------
    pd.Series  index=날짜(Timestamp), values=코스피 종가지수
    """
    df  = pd.read_excel(filepath, header=None)
    raw_dates = df.iloc[14:, 0].reset_index(drop=True)
    dates_idx = pd.to_datetime(raw_dates.astype(str).str[:10], errors='coerce')
    kospi_vals = df.iloc[14:, 2].values.astype(float)
    return pd.Series(kospi_vals, index=dates_idx, name='코스피')
...
def build_switch_table(sector_info: dict,
    ...
    """
    섹터 이동 종목별 gain 및 벤치마크 대비 초과 주가 수익률 계산.
    ...
    ────────────────────────
        ...
        벤치마크 수익률 = (kospi_T - kospi_T1) / kospi_T1   ← 코스피 지수 당일/전날 수익률
    ...
    for date_str, info in sector_info.items():
        ...
        # ── 종가 시리즈 ──────────────────────────────────────
        price_T  = item_dfs['종가'].loc[cur_date]      # T 당일 종가
        price_T1 = item_dfs['종가'].loc[prev_biz_day]  # T-1 전날 종가

        # ── 벤치마크(코스피) 수익률 계산 ──────────────────────
        bm_T  = float(kospi_index.get(cur_date,     np.nan))
        bm_T1 = float(kospi_index.get(prev_biz_day, np.nan))
        if not np.isnan(bm_T1) and bm_T1 > 0 and not np.isnan(bm_T):
            bm_ret = (bm_T - bm_T1) / bm_T1   # 코스피 당일 수익률
        else:
            bm_ret = np.nan
        ...
        for code in common:
            ...
            if not np.isnan(p_t1) and p_t1 > 0 and not np.isnan(p_t):
                stock_ret = (p_t - p_t1) / p_t1          # 종목 수익률
            else:
                stock_ret = np.nan

            # 초과 수익률 = 종목 수익률 - 코스피 벤치마크 수익률
            if not np.isnan(stock_ret) and not np.isnan(bm_ret):
                excess_ret = stock_ret - bm_ret
                excess_win = int(excess_ret > 0)          # 초과 승률 기준
            else:
                excess_ret = np.nan
                excess_win = np.nan
            ...
            records.append({
                ...
                '코스피수익률(벤치마크)':    round(bm_ret,    6) if not np.isnan(bm_ret)    else np.nan,
...
def save_csvs(switch_df: pd.DataFrame,
    ...
    for date_str in sorted(switch_df['정기변경일'].unique()):
        ...
        path = os.path.join(output_dir,
                            f"kospi_sector_switch_{date_str.replace('-','')}{suffix}.csv")
        sub.to_csv(path, index=False, encoding='utf-8-sig')
        saved.append(path)

    path_all = os.path.join(output_dir, f"kospi_sector_switch_all{suffix}.csv")
    switch_df.to_csv(path_all, index=False, encoding='utf-8-sig')
    saved.append(path_all)

    path_gain = os.path.join(output_dir, f"kospi_gain_analysis{suffix}.csv")
...
def main():
    ...
    print(f"  [SETTING]  BENCHMARK_PATH  = {BENCHMARK_PATH}")
    print(f"{'='*80}")

    # ── 데이터 로딩 ───────────────────────────────────────────
    item_dfs, all_dates_sorted, code_to_name = load_data(DATA_PATH)
    kospi_index = load_benchmark(BENCHMARK_PATH)
    print(f"  벤치마크(코스피 지수) 로딩 완료: {len(kospi_index)}일")
    ...
    switch_df, gain_df = build_switch_table(
        sector_info, aum_dict, item_dfs, code_to_name, kospi_index
    )

    # ────────────────────────────────────────────────────────
    # 출력 1: 리밸런싱별 섹터 AUM
    # ────────────────────────────────────────────────────────
    _print_section("① 리밸런싱별 섹터별 추정 AUM  (섹터유지종목 교집합 표본)")
    ...
    outlier_note = ("※ 이상치: x·y<0 표본 제거"
                    if REMOVE_OUTLIERS else "※ 이상치 제거 없음")
    print(f"\n  {outlier_note}")
    print(f"  ※ gain: val_old = AUM(당일, s_old) × 비중(직전 정기변경일)")
    print(f"           val_new = AUM(당일, s_new) × 비중(당일)")

    # ────────────────────────────────────────────────────────
    # 출력 2: 리밸런싱별 케이스별 — 종목수 · 이득확률 · 초과승률 · 평균초과수익률
    # ────────────────────────────────────────────────────────
    _print_section("② 리밸런싱별 케이스별 분석  (주가수익률 = 코스피 대비 초과수익률)")

    # 리밸런싱일별 코스피 수익률 출력
    print(f"  {'날짜':<14}  코스피 당일 수익률")
    print('─'*40)
    for d in dates_list:
        t1 = sector_info[d]['prev_biz_day_str']
        bT  = float(kospi_index.get(pd.Timestamp(d),  np.nan))
        bT1 = float(kospi_index.get(pd.Timestamp(t1), np.nan))
    ...
    # 출력 3: 전체 기간 합산
    # ────────────────────────────────────────────────────────
    _print_section("③ 전체 기간 합산 — 케이스별 분석  (코스피 대비 초과수익률 기준)")
    ...
    # ────────────────────────────────────────────────────────
    # 출력 4: 리밸런싱별 케이스별 종목 수
    # ────────────────────────────────────────────────────────
    _print_section("④ 리밸런싱별 케이스별 종목 수")
    header = (f"  {'날짜':<14}"
...
#python kospi_정기변경/final_kospi.py