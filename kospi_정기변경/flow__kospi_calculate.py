...
def run_regression(item_dfs: dict, all_dates_sorted: list):
    ...
    for idx in range(1, len(REBAL_DATES)):
        ...
        weights_old       = calc_sector_weights(cap_prev_biz,   sector_old)  # 회귀 x변수용 (T-1 cap)
        weights_old_rebal = calc_sector_weights(cap_prev_rebal, sector_old)  # gain val_old 비중용
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 80)
    print(f"▶ 회귀 결과 (REMOVE_OUTLIERS={REMOVE_OUTLIERS}, 표본=섹터유지종목)")
    print("=" * 80)
    print(results_df.to_string(index=False))

#python kospi_정기변경/flow__kospi_calculate.py